...
contract PolyrollERC20 is VRFConsumerBase, Ownable, ReentrancyGuard {
    ...
    uint constant CHAINLINK_FEE = 100000000000000; // 0.0001 LINK
    ...
    address public constant ROLL_MATIC_LP = 0x905DCc700fcce9a49b7D907E371230995a45ebCE;
    ...
    function maxProfit() public view returns (uint) {
        return IERC20(gameToken).balanceOf(address(this)) / balanceMaxProfitRatio;
    }
    ...
    function setMiner(IMiner _miner) external onlyPolyroll {
        miner = _miner;
    }
    ...
    function setRewardPct(uint _rewardPct) external onlyPolyroll {
        require(_rewardPct <= 100, "rewardPct exceeds 100%");
        rewardPct = _rewardPct;
    }

    // Set maximum ROLL reward that a user can receive per bet.
    function setMaxRollReward(uint _maxRollReward) external onlyPolyroll {
    ...
    function updateRollGameRatio() internal {
        ...
        if (block.timestamp - rollGameBlockTimestampLast >= 1800) {
            ...
            if (rollGameRatio == 0) {
                // Update rollGameRatio if first time updating.
                rollGameRatio = newRollGameRatio;
            } else if (SAFETY_FACTOR * newRollGameRatio  / rollGameRatio > SAFETY_FACTOR * 110 / 100) {
                // Cap any increase to +10% per update.
                rollGameRatio = rollGameRatio * 110 / 100;
            } else if (SAFETY_FACTOR * newRollGameRatio / rollGameRatio < SAFETY_FACTOR * 90 / 100) {
                // Cap any decrease to -10% per update.
                rollGameRatio = rollGameRatio * 90 / 100;
            } else {
                rollGameRatio = newRollGameRatio;
            }
            rollGameBlockTimestampLast = block.timestamp;
        }
    }
    ...
    function placeBet(uint256 amount, uint betMask, uint modulo, address referrer) external nonReentrant {
        ...
        // Validate input data.
        require(LINK.balanceOf(address(this)) >= CHAINLINK_FEE, "Insufficient LINK token");
        ...
        require(lockedInBets + possibleWinAmount <= IERC20(gameToken).balanceOf(address(this)), "Insufficient funds");

        // Update lock funds.
        lockedInBets += possibleWinAmount;

        // Request random number from Chainlink VRF. Store requestId for validation checks later.
        bytes32 requestId = requestRandomness(KEY_HASH, CHAINLINK_FEE, bets.length);
    ...
    function refundBet(uint betId) external nonReentrant {
        ...
        require(amount > 0, "Bet does not exist");
        require(bet.isSettled == false, "Bet is settled already");
        require(block.number > bet.placeBlockNumber + 10000, "Wait 6 hours");
        ...
        // Send the refund.
        IERC20(gameToken).safeTransfer(address(bet.gambler), amount);
